...
class CameraSource(
    ...
    suspend fun initCamera() {
        ...
        imageReader?.setOnImageAvailableListener({ reader ->
            ...
            if (image != null) {
                ...
                /**プレビューを回転させる（フロントにすると逆さになるので）90f→270f 原因は謎**/
    ...
    /**利用するカメラをバックかフロントか決める**/
    fun prepareCamera() {

        for (cameraId in cameraManager.cameraIdList) {

            /**0か1が取得できる　0がバック、1がフロント**/
    ...
    private fun processImage(bitmap: Bitmap) {
        ...
        synchronized(lock) {
            detector?.estimatePoses(bitmap)?.let {
                Log.d("--pose","id:${it[0].id} keyPoints:${it[0].keyPoints[0].bodyPart}, score:${it[0].score}")
                persons.addAll(it)
